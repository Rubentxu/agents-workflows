...
export function Sidebar() {
  ...
  return (
    <div className="h-full flex flex-col bg-surface border-r border-outline">
      ...
      <div className="px-4 py-2 border-b border-outline-variant">
        <button
          ...
          disabled={loading}
          className="w-full py-2 px-3 text-sm bg-surface-container hover:bg-surface-container-high rounded font-medium text-on-surface/70 transition-colors disabled:opacity-50"
        >
...
function WorkflowList({ onNodeClick }: WorkflowListProps) {
  ...
  if (workflows.length === 0) {
    return (
      <div className="px-4 py-8 text-center text-sm text-on-surface/40">
        No workflows found.
        <br />
          ...
          <div key={type}>
            ...
            {grouped[type].map((node) => (
              ...
              >
                ...
                <div className="font-mono text-xs text-on-surface/40 truncate mt-0.5">
                  {node.namespace}
                </div>
              </button>
            ))}
