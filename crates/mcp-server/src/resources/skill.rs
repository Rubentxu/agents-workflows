...
pub async fn list(state: Arc<AppState>) -> Result<Json<serde_json::Value>, CrudError> {
    let nodes = state.list_by_type_str("skill").map_err(internal_error)?;
...
pub async fn update(
    ...
    let arn = validate_arn(arn)?;
    let existing = state.get_node_by_arn(&arn).map_err(internal_error)?.ok_or_else(|| not_found("Skill", &arn))?;
